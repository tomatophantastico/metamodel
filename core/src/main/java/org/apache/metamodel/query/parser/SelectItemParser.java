...
public final class SelectItemParser implements QueryPartProcessor {
    ...
    /**
     * Finds/creates a SelectItem based on the given expression. Unlike the {@link #parse(String, String)} method, this
     * method will not actually add the selectitem to the query.
     * 
     * @param expression
     * @return
     * 
     * @throws MultipleSelectItemsParsedException if an expression yielding multiple select-items (such as "*") was
     *             passed in the expression
     */
            ...
            if (function != null) {
                ...
                if (function instanceof CountAggregateFunction && "*".equals(expression)) {
                    ...
                    if (expressionSplit.length > 1) {
                        ...
                        functionParameters =
                                Arrays.copyOfRange(expressionSplit, 1, expressionSplit.length, String[].class);
                    }
                }
            }
        } else {
