...
public class WhereBuilderImplTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        query = new Query();
        GroupedQueryBuilder queryBuilder = new GroupedQueryBuilderImpl(null, query);
    ...
    public void testOr() throws Exception {
        ...
        assertEquals(" WHERE (col1 = TRUE OR col2 LIKE '%testcase%')", query.toSql());
    }

    public void testNotLike() throws Exception {
        whereBuilder.eq(true).or(col2).notLike("%test%case%");

        assertEquals(" WHERE (col1 = TRUE OR col2 NOT LIKE '%test%case%')", query.toSql());
    }

    public void testAnd() throws Exception {
        whereBuilder.differentFrom(true).and(col2).eq(1).or(col2).eq(2).or(col2).eq(3).and(new MutableColumn("col3"))
                .eq(4);

        assertEquals(" WHERE col1 <> TRUE AND (col2 = 1 OR col2 = 2 OR col2 = 3) AND col3 = 4", query.toSql());
    }

    public void testInStringArray() throws Exception {
        whereBuilder.eq(true).or(col2).in("foo", "bar");

        assertEquals(" WHERE (col1 = TRUE OR col2 IN ('foo' , 'bar'))", query.toSql());
    }

    public void testNotInStringArray() throws Exception {
        whereBuilder.eq(true).or(col2).notIn("foo", "bar");

        assertEquals(" WHERE (col1 = TRUE OR col2 NOT IN ('foo' , 'bar'))", query.toSql());
    }
